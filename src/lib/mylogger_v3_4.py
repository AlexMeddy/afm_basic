...
ansi_escape_colors = {
    ...
    34: "\033[36m", #black letter white BG
...
{ansi_escape_colors[COLOR_RESET]}({args_desc})\
        line {ansi_escape_colors[16]}{inspect.currentframe().f_back.f_lineno}{ansi_escape_colors[COLOR_RESET]}'      
            ...
            myprint(msg2)     
            dictionary_previous_items_list = str(dictionary_previous).split(',')
            dictionary_previous_list = []
            j=0
            list_size = math.trunc (len (str(dictionary_previous)) / 100) + 1
            for i in range (list_size):
                dictionary_previous_list.append('')
                while len (dictionary_previous_list[i]) + len(dictionary_previous_items_list[i]) < 100 and j < len(dictionary_previous_items_list):
                    if (dictionary_previous_list[i] != ''):
                        dictionary_previous_list[i] += ','
                    dictionary_previous_list[i] += (dictionary_previous_items_list[j])
                    j+=1
                if  i < (list_size-1):
                    dictionary_previous_list[i] += ','
    
                    
            #for i in dictionary_previous_list:
            #    print (i)
            if (log_level_p & DISPLAY_STATE):

                for i, val in enumerate(dictionary_previous_list):

                
                    msg1 = f'{pad*indent*indent_mult}{ansi_escape_colors[indent]}|{ansi_escape_colors[20]}{'State   ' if i==0 else '        '}\
    {ansi_escape_colors[COLOR_RESET]}{ansi_escape_colors[COLOR_RESET]}{':' if i==0 else ' '}{val}\
    {ansi_escape_colors[COLOR_RESET]}'   
            ...
            dictionary_post_org = dictionary_post
            for i in inspect.getmembers(args[0]):
                if not i[0].startswith('_'):
                    if not inspect.ismethod(i[1]): 
                        #myprint('---------------',i[0],i[1])
...
{escape_color}{differences}{ansi_escape_colors[COLOR_RESET]}'
                ...
                myprint(msg3)
                dictionary_post_items_list = str(dictionary_post_org).split(',')

                dictionary_post_list = []
                j=0
                list_size = math.trunc (len (str(dictionary_post_org)) / 100) + 1
                for i in range (list_size):
                    dictionary_post_list.append('')
                    while len (dictionary_post_list[i]) + len(dictionary_post_items_list[i]) < 100 and j < len(dictionary_post_items_list):
                        if (dictionary_post_list[i] != ''):
                            dictionary_post_list[i] += ','
                        dictionary_post_list[i] += (dictionary_post_items_list[j])
                        j+=1



                for i, val in enumerate(dictionary_post_list):
                    msg1 = f'{pad*indent*indent_mult}{ansi_escape_colors[indent]}|{ansi_escape_colors[20]}{'NewState' if i==0 else '        '}\
    {ansi_escape_colors[COLOR_RESET]}{ansi_escape_colors[COLOR_RESET]}{':' if i==0 else ' '}{val}\
    {ansi_escape_colors[COLOR_RESET]}'   
                    msg1=add_tree_ident(msg1)
                    #msg1=replace_and_modify_substring(msg1, '\'guid\': ', ',',ansi_escape_colors[20],ansi_escape_colors[COLOR_RESET])
...
myic:{ansi_escape_colors[COLOR_RESET]}'
    ...
    for i, arg in enumerate(args):
        ...
        msg += f'{ansi_escape_colors[MYIC_COLOR]}{param_text[i]}{ansi_escape_colors[COLOR_RESET]}'
        if arg != param_text[i]:
            msg += f'={ansi_escape_colors[MYIC_COLOR]}{arg}{ansi_escape_colors[COLOR_RESET]}'
    msg = truncate_memory_address(msg)
   
    msg += f'      line {ansi_escape_colors[COLOR_RESET]}{inspect.currentframe().f_back.f_lineno}{ansi_escape_colors[COLOR_RESET]}'
